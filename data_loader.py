...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        with self.driver.session() as session:
            # clean slate so counts are deterministic
            session.run("MATCH (n) DETACH DELETE n")

            # ensure one node per Location id
            session.run("""
            ...
            # load every row from the CSV and create TRIP relationships
            csv_url = f"file:///{file_path.split('.')[0]}.csv"
            cypher = """
            CALL {
              LOAD CSV WITH HEADERS FROM $csv AS row
              WITH
                toInteger(row.PULocationID) AS pu,
                toInteger(row.DOLocationID) AS do,
                toFloat(row.trip_distance)  AS dist,
                toFloat(row.fare_amount)    AS fare,
                localdatetime(replace(row.tpep_pickup_datetime,' ','T'))  AS pdt,
                localdatetime(replace(row.tpep_dropoff_datetime,' ','T')) AS ddt
              MERGE (src:Location {name: pu})
              MERGE (dst:Location {name: do})
              CREATE (src)-[:TRIP {
                distance:  dist,
                fare:      fare,
                pickup_dt: pdt,
            ...
            RETURN sum(c) AS loaded
            """
            session.run(cypher, {"csv": csv_url})
